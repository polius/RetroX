...
// Hat-axis decoder lives in gamepad-hat.js (shared with play.js — the
// in-game directional path needs it to make the D-pad work for users
// on Firefox + Sony non-standard mappings).
...
function pollGamepad() {
  ...
  // Controller-bindings dialog: while the user is capturing a button to
  // remap, every gamepad button (incl. B/Circle and A/Cross) must be
  // available as a candidate — including the one we'd normally treat as
  // "close the modal". Honor a global capture flag set by the rebind
  // dialog and let it own the pad until capture completes or cancels.
  //
  // We still tick the rising-edge state map below so that when capture
  // ends, a button the user is still holding doesn't register as a
  // "fresh press" the very next frame — that's how the Circle-pressed-
  // during-capture-bound used to dismiss the modal: capture cleared the
  // flag, gamepad-nav resumed, prev still said the button was up, and
  // the still-held Circle fired btnPressed → back gesture.
  if (window.__retroxRebindCapture) {
    for (const gp of pads) {
      if (!gp) continue;
      for (let i = 0; i < gp.buttons.length; i++) {
        const v = gp.buttons[i];
        const cur = !!(v && (typeof v === "object" ? v.pressed : v > 0.5));
        prev.set(`${gp.index}:${i}`, cur);
      }
    }
    requestAnimationFrame(pollGamepad);
    return;
